...
export function FollowUpModelsManager({ isDark, workspaceId, categoriesRefreshTrigger }: FollowUpModelsManagerProps) {
  ...
  const [selectedCategoryFilter, setSelectedCategoryFilter] = useState<string>(''); // ✅ Filtro por categoria
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Load categories - ✅ FILTRAR POR WORKSPACE
  ...
  // ✅ Filtrar modelos pela categoria selecionada
  const filteredModels = selectedCategoryFilter
    ? models.filter(model => model.category_id === selectedCategoryFilter)
    : models;
    ...
    <div className="space-y-6">
      ...
      {categories.length > 0 && models.length > 0 && (
        <div className="flex items-center gap-3">
          <Label className={cn("text-sm", isDark ? "text-white/70" : "text-zinc-700")}>
