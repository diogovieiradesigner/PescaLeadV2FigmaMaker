import { Search, Bell, ChevronDown, LayoutGrid, List, Sun, Moon, Settings, RefreshCw, Menu, Filter } from 'lucide-react';
...
interface HeaderProps {
  ...
  onMobileMenuClick?: () => void; // ✅ NOVO: Callback para abrir menu mobile
  onFiltersToggle?: () => void; // ✅ NOVO: Callback para toggle dos filtros
  showFilters?: boolean; // ✅ NOVO: Estado de visibilidade dos filtros
}
...
export function Header({
  ...
  onMobileMenuClick, // ✅ NOVO
  onFiltersToggle, // ✅ NOVO
  showFilters, // ✅ NOVO
}: HeaderProps) {
  ...
  return (
    <header className={`h-16 border-b flex items-center justify-between px-4 md:px-6 transition-colors ${
      isDark 
      ...
      <div className="hidden md:flex items-center gap-4">
        ...
        {/* Filters Toggle Button - only show in kanban view */}
        {viewMode === 'kanban' && onFiltersToggle && (
          <button
      ...
      {/* Center Section - Search (Desktop & Mobile) */}
      ...
      <div className="flex items-center gap-2 md:gap-4">
        ...
        <button className={`hidden md:flex relative p-1.5 rounded-lg transition-colors ${
          isDark 
            ? 'hover:bg-white/[0.05]' 
            : 'hover:bg-light-elevated-hover'
        }`}>
        ...
        {/* Refresh Leads - Hidden on mobile */}
        {onRefresh && (
          <button
