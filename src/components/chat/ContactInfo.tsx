import { MessageSquare, X, User, Bot, ChevronDown, ChevronUp, DollarSign, Calendar, Tag, User as UserIcon, Mail, Phone, CheckCircle, Globe, Send } from 'lucide-react';
...
function renderEmailValueCompact(email: string, isDark: boolean) {
  return (
  ...
  if (fieldNameLower.includes('email') && fieldNameLower.includes('domínio')) {
    try {
      ...
      return (
        <div className={`text-sm flex items-center gap-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
          <Mail className="w-3 h-3 opacity-50" />
          <span className="truncate flex-1">{domainEmail.address}</span>
      ...
      // Se não for JSON, verificar se é e-mail simples
      if (isValidEmail(fieldValue)) {
        return renderEmailValueCompact(fieldValue, isDark);
      }
      return fieldValue;
    }
  }
  
  // ✅ Verificar se é e-mail simples (string) quando o campo contém "email" no nome
  if (fieldNameLower.includes('email') && isValidEmail(fieldValue)) {
    return renderEmailValueCompact(fieldValue, isDark);
  }
