...
export async function getColumnLeads(
  ...
  console.log('[KANBAN HELPERS] getColumnLeads chamado com filtros:', { workspaceId, funnelId, columnId, offset, limit, filters });
  ...
  if (filters?.hasEmail) {
    console.log('[KANBAN HELPERS] Aplicando filtro: hasEmail');
    // Filtrar leads que possuem email (não null)
    countQuery = countQuery.not('email', 'is', null);
    leadsQuery = leadsQuery.not('email', 'is', null);
  }
  
  if (filters?.hasWhatsapp) {
    console.log('[KANBAN HELPERS] Aplicando filtro: hasWhatsapp');
    // Filtrar leads que possuem telefone (não null)
  ...
  if (filters?.searchQuery?.trim()) {
    ...
    console.log('[KANBAN HELPERS] Aplicando filtro: searchQuery =', search);
  ...
  console.log('[KANBAN HELPERS] Executando query de contagem...');
  const { count, error: countError } = await countQuery;
  
  if (countError) {
    console.error('[KANBAN HELPERS] ❌ Error counting leads:', {
      error: countError,
    ...
    throw new Error(`Failed to count leads: ${countError.message || countError.code || JSON.stringify(countError)}`);
  }
  
  const total = count ?? 0;
  console.log('[KANBAN HELPERS] Total de leads (com filtros):', total);
  ...
  console.log('[KANBAN HELPERS] Leads carregados:', leads?.length || 0);
