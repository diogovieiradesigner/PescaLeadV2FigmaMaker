...
const UazapiProvider: IWhatsAppProvider = {
  ...
  async sendTextMessage({ instanceName, token, number, text, quoted }) {
    ...
    try {
      ...
      console.log('[Uazapi] ✅ Checking for message ID in response...');
      console.log('[Uazapi]    Response keys:', Object.keys(data));
      console.log('[Uazapi]    data.id:', data.id);
      console.log('[Uazapi]    data.messageId:', data.messageId);
      console.log('[Uazapi]    data.data?.id:', data.data?.id);
      
      // ✅ IMPORTANTE: Normalizar resposta para compatibilidade com Evolution
      // Se UAZAPI retornar { id: "..." }, mapear para { key: { id: "..." } }
      if (data.id && !data.key) {
        console.log('[Uazapi] 🔄 Normalizing response to Evolution format');
        data.key = { id: data.id };
      }
  ...
  async deleteMessage({ instanceName, token, messageId, remoteJid, fromMe, participant }) {
    // ✅ CORREÇÃO: Usar o token da instância (não o ADMIN_TOKEN)
    ...
    // ✅ Payload para UAZAPI: { id: "messageId" }
    const payload = {
      id: messageId  // ✅ UAZAPI usa "id", não "messageId"
    };
    ...
    try {
      const response = await fetch(`${baseUrl}/message/delete`, {  // ✅ URL correta
        method: 'POST',  // ✅ Método correto
        headers: { 
          'token': token,  // ✅ CORREÇÃO: Usa token da instância (não admintoken)
          'Content-Type': 'application/json',
          'Accept': 'application/json'
        },
        body: JSON.stringify(payload)
