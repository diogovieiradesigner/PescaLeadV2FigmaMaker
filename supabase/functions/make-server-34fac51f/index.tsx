...
console.log('🚀 [SERVER] Build: 2025-12-02-CLEAN-START');
...
async function validateAuth(c: any, next: any) {
  ...
  c.set('user', user); // Adicionar objeto user completo
...
// ============================================
// ADMIN ROUTES - REMOVIDO TEMPORARIAMENTE
// ============================================
...
async function validateSuperAdmin(c: any, next: any) {
  const userId = c.get('userId');
  ...
  if (error || !superAdmin) {
    console.log('[ADMIN] Access denied - not a super admin:', userId);
...
// Test route to verify admin routes are reachable
app.get('/make-server-e4f9d774/admin-test', (c) => {
...
// 🧪 TEST: Endpoint simples para testar se servidor está respondendo
app.get('/admin-test', (c) => {
  console.log('🧪 [ADMIN-TEST-NEW] New test endpoint called!');
  return c.json({ 
    status: 'ok', 
    ...
    routes: [
      ...
      'POST /make-server-34fac51f/admin-delete-user'
    ]
  });
});

// 🧪 TEST: Endpoint POST sem auth para testar CORS
app.post('/admin-test-post', async (c) => {
  console.log('🧪 [ADMIN-TEST-POST] POST endpoint called!');
  const body = await c.req.json();
  console.log('🧪 [ADMIN-TEST-POST] Body received:', body);
  return c.json({ 
    status: 'ok', 
    message: 'POST works correctly',
    received: body
  });
});
...
// ============================================
app.post('/make-server-34fac51f/admin-delete-user', validateAuth, validateSuperAdmin, async (c) => {
  console.log('🗑️ [ADMIN-DELETE-USER] ========== START ==========');
  
  try {
    const { user_id, hard_delete } = await c.req.json();
    const adminUserId = c.get('userId');
    const adminEmail = c.get('userEmail');
    
    console.log('🗑️ [ADMIN-DELETE-USER] Request from admin:', { adminUserId, adminEmail });
    console.log('🗑️ [ADMIN-DELETE-USER] Target user_id:', user_id);
    console.log('🗑️ [ADMIN-DELETE-USER] Hard delete:', hard_delete);

    // Validation
    if (!user_id) {
      console.error('🗑️ [ADMIN-DELETE-USER] ERROR: Missing user_id');
      return c.json({ success: false, error: 'user_id is required' }, 400);
    }

    // Step 1: Get user details before deletion
    console.log('🗑️ [ADMIN-DELETE-USER] Step 1: Fetching user details from auth.users...');
    const { data: authUser, error: getUserError } = await supabase.auth.admin.getUserById(user_id);
    
    if (getUserError) {
      console.error('🗑️ [ADMIN-DELETE-USER] ERROR fetching auth user:', getUserError);
      return c.json({ 
        success: false, 
        error: `Failed to fetch user from auth: ${getUserError.message}`,
        details: getUserError 
      }, 500);
    }

    if (!authUser?.user) {
      console.error('🗑️ [ADMIN-DELETE-USER] ERROR: User not found in auth.users');
      return c.json({ success: false, error: 'User not found in authentication system' }, 404);
    }

    console.log('🗑️ [ADMIN-DELETE-USER] User found in auth:', {
      id: authUser.user.id,
      email: authUser.user.email,
      created_at: authUser.user.created_at
    });

    // Step 2: Delete from workspace_members table
    console.log('🗑️ [ADMIN-DELETE-USER] Step 2: Deleting from workspace_members...');
    ...
    if (deleteMembersError) {
      console.error('🗑️ [ADMIN-DELETE-USER] ERROR deleting workspace_members:', deleteMembersError);
      return c.json({ 
        success: false, 
        error: `Database error deleting user memberships: ${deleteMembersError.message}`,
        details: deleteMembersError,
        step: 'workspace_members'
      }, 500);
    }

    console.log('🗑️ [ADMIN-DELETE-USER] ✅ Deleted from workspace_members');

    // Step 3: Delete from users_34fac51f table
    console.log('🗑️ [ADMIN-DELETE-USER] Step 3: Deleting from users_34fac51f...');
    ...
    if (deleteUsersError) {
      console.error('🗑️ [ADMIN-DELETE-USER] ERROR deleting from users_34fac51f:', deleteUsersError);
      return c.json({ 
        success: false, 
        error: `Database error deleting user: ${deleteUsersError.message}`,
        details: deleteUsersError,
        step: 'users_34fac51f'
      }, 500);
    }

    console.log('🗑️ [ADMIN-DELETE-USER] ✅ Deleted from users_34fac51f');

    // Step 4: Hard delete from auth.users (if requested)
    if (hard_delete) {
      console.log('🗑️ [ADMIN-DELETE-USER] Step 4: Hard deleting from auth.users...');
      const { error: deleteAuthError } = await supabase.auth.admin.deleteUser(user_id);

      if (deleteAuthError) {
        console.error('🗑️ [ADMIN-DELETE-USER] ERROR deleting from auth.users:', deleteAuthError);
        return c.json({ 
          success: false, 
      ...
      console.log('🗑️ [ADMIN-DELETE-USER] ✅ Hard deleted from auth.users');
    }

    // Success!
    console.log('🗑️ [ADMIN-DELETE-USER] ========== SUCCESS ==========');
    return c.json({ 
      success: true, 
    ...
    console.error('🗑️ [ADMIN-DELETE-USER] ========== UNEXPECTED ERROR ==========');
...
// ============================================
app.post('/admin-create-user', validateAuth, validateSuperAdmin, async (c) => {
  ...
  try {
    ...
    console.log('👤 [ADMIN-CREATE-USER] Request from admin:', { adminUserId, adminEmail });
    console.log('👤 [ADMIN-CREATE-USER] New user data:', { name, email, phone: phone || 'not provided' });

    // Validation
    if (!name || !email || !password) {
      console.error('👤 [ADMIN-CREATE-USER] ERROR: Missing required fields');
      return c.json({ 
        success: false, 
    ...
    console.log('👤 [ADMIN-CREATE-USER] Step 1: Creating user in auth.users...');
    const { data: authData, error: authError } = await supabase.auth.admin.createUser({
    ...
    if (!authData?.user) {
      console.error('👤 [ADMIN-CREATE-USER] ERROR: No user returned from auth');
      return c.json({ 
        success: false, 
    ...
    console.log('👤 [ADMIN-CREATE-USER] ✅ User created in auth.users:', authData.user.id);

    // Step 2: Create user in users_34fac51f table
    console.log('👤 [ADMIN-CREATE-USER] Step 2: Creating user in users_34fac51f...');
    ...
    if (userError) {
      console.error('👤 [ADMIN-CREATE-USER] ERROR creating user in users_34fac51f:', userError);
      
      // Rollback: Delete auth user
      console.log('👤 [ADMIN-CREATE-USER] Rolling back: Deleting auth user...');
      await supabase.auth.admin.deleteUser(authData.user.id);
      
      return c.json({ 
        success: false, 
        error: `Database error creating user: ${userError.message}`,
        details: userError,
        step: 'users_34fac51f'
      }, 500);
    }

    console.log('👤 [ADMIN-CREATE-USER] ✅ User created in users_34fac51f');
...
// ============================================
app.post('/admin-alterar-senha', validateAuth, validateSuperAdmin, async (c) => {
  console.log('🔑 [ADMIN-CHANGE-PASSWORD] ========== START ==========');
  ...
  try {
    ...
    console.log('🔑 [ADMIN-CHANGE-PASSWORD] Request from admin:', { adminUserId, adminEmail });
    console.log('🔑 [ADMIN-CHANGE-PASSWORD] Target user_id:', user_id);
    console.log('🔑 [ADMIN-CHANGE-PASSWORD] New password length:', new_password?.length);

    // Validation
    if (!user_id) {
      console.error('🔑 [ADMIN-CHANGE-PASSWORD] ERROR: Missing user_id');
      return c.json({ 
        success: false, 
    ...
    if (!new_password) {
      console.error('🔑 [ADMIN-CHANGE-PASSWORD] ERROR: Missing new_password');
      return c.json({ 
        success: false, 
    ...
    if (new_password.length < 6) {
      console.error('🔑 [ADMIN-CHANGE-PASSWORD] ERROR: Password too short');
      return c.json({ 
        success: false, 
    ...
    console.log('🔑 [ADMIN-CHANGE-PASSWORD] Step 1: Verifying user exists...');
    const { data: existingUser, error: getUserError } = await supabase.auth.admin.getUserById(user_id);

    if (getUserError) {
      console.error('🔑 [ADMIN-CHANGE-PASSWORD] ERROR getting user:', getUserError);
      return c.json({ 
        success: false, 
    ...
    if (!existingUser?.user) {
      console.error('🔑 [ADMIN-CHANGE-PASSWORD] ERROR: User not found');
      return c.json({ 
        success: false, 
    ...
    console.log('🔑 [ADMIN-CHANGE-PASSWORD] ✅ User found:', {
      id: existingUser.user.id,
      email: existingUser.user.email
    });

    // Step 2: Update password
    console.log('🔑 [ADMIN-CHANGE-PASSWORD] Step 2: Updating password...');
    ...
    if (updateError) {
      console.error('🔑 [ADMIN-CHANGE-PASSWORD] ERROR updating password:', updateError);
      return c.json({ 
        success: false, 
    ...
    // Success!
    console.log('🔑 [ADMIN-CHANGE-PASSWORD] ========== SUCCESS ==========');
    return c.json({ 
      success: true, 
    ...
    console.error('🔑 [ADMIN-CHANGE-PASSWORD] ========== UNEXPECTED ERROR ==========');
...
console.log('  - POST /admin-alterar-senha');
console.log('  - POST /make-server-34fac51f/admin-delete-user');
