-- =============================================================================
-- MIGRATION: Configure Instagram tables for cascade delete with orphaned leads
-- =============================================================================
-- Esta migration configura:
-- - Função RPC para deletar extrações Instagram com cascata
-- - Apenas para tabelas compartilhadas (lead_extraction_runs, lead_extraction_staging, etc)
-- - Os leads criados ficam órfãos (não são deletados)
-- =============================================================================
...
-- =============================================================================
-- FK CONSTRAINTS para tabelas compartilhadas
-- =============================================================================

-- 1. Garantir CASCADE em extraction_logs -> lead_extraction_runs
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = 'extraction_logs_run_id_fkey'
        AND table_name = 'extraction_logs'
    ) THEN
        ALTER TABLE extraction_logs
            DROP CONSTRAINT extraction_logs_run_id_fkey;
        ALTER TABLE extraction_logs
...
-- 2. Garantir CASCADE em lead_extraction_staging -> lead_extraction_runs
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = 'lead_extraction_staging_extraction_run_id_fkey'
        AND table_name = 'lead_extraction_staging'
    ) THEN
        ALTER TABLE lead_extraction_staging
            DROP CONSTRAINT lead_extraction_staging_extraction_run_id_fkey;
        ALTER TABLE lead_extraction_staging
            ADD CONSTRAINT lead_extraction_staging_extraction_run_id_fkey
            FOREIGN KEY (extraction_run_id) REFERENCES lead_extraction_runs(id) ON DELETE CASCADE;
    END IF;
END $$;

-- 3. Garantir CASCADE em watchdog_logs -> lead_extraction_runs
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints
...
-- =============================================================================
-- RLS policy para DELETE em lead_extraction_runs
-- =============================================================================
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies
        WHERE tablename = 'lead_extraction_runs'
        AND policyname = 'Workspace members can delete runs'
    ) THEN
        CREATE POLICY "Workspace members can delete runs" ON lead_extraction_runs
            FOR DELETE USING (
                workspace_id IN (
