-- =============================================================================
-- Migration: Fix extraction analytics RPCs
-- =============================================================================
-- Esta migration:
-- 1. Atualiza get_extraction_analytics (Google Maps) adicionando campo 'source'
-- 2. Cria/atualiza get_instagram_extraction_analytics para Instagram
-- =============================================================================

-- =============================================================================
-- PARTE 1: Atualizar get_extraction_analytics (Google Maps/CNPJ)
-- Adiciona campo 'source' ao retorno para detecção correta no frontend
-- =============================================================================
...
BEGIN
  ...
  -- ========================================
  -- DADOS DA RUN (agora inclui source)
  -- ========================================
...
-- =============================================================================
-- PARTE 2: Criar/Atualizar get_instagram_extraction_analytics
-- =============================================================================

CREATE OR REPLACE FUNCTION get_instagram_extraction_analytics(run_id UUID)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  ...
  v_profiles_migrated INT := 0;
  v_profiles_pending INT := 0;
  v_profiles_failed INT := 0;
  v_profiles_skipped INT := 0;

  -- Métricas de perfis
  v_verified_count INT := 0;
  v_business_count INT := 0;
  v_professional_count INT := 0;
  v_private_count INT := 0;
  v_public_count INT := 0;

  -- Contatos
  v_with_email INT := 0;
  v_with_phone INT := 0;
  v_with_website INT := 0;
  v_with_whatsapp INT := 0;
  v_email_from_bio INT := 0;
  v_phone_from_bio INT := 0;

  -- Métricas de seguidores
  v_avg_followers NUMERIC := 0;
  v_max_followers INT := 0;
  v_min_followers INT := 0;

BEGIN
  -- Verificar se run existe e é do Instagram
  IF NOT EXISTS (
    SELECT 1 FROM lead_extraction_runs
    WHERE id = v_run_id AND source = 'instagram'
  ) THEN
    RETURN json_build_object('error', 'Run não encontrada ou não é do Instagram');
  END IF;

  -- ========================================
  -- DADOS DA RUN (compatível com Google Maps)
  -- ========================================
  SELECT json_build_object(
    'id', r.id,
    'extraction_id', r.extraction_id,
    'workspace_id', r.workspace_id,
    'source', r.source,
    'run_name', COALESCE(r.run_name, r.niche || ' - ' || r.location),
    'search_term', r.niche,
    'niche', r.niche,
    'location', r.location,
    'status', r.status,
    ...
    'discovery_queries_total', r.discovery_queries_total,
    'discovery_queries_completed', r.discovery_queries_completed,
    'discovery_profiles_found', r.discovery_profiles_found,
    'discovery_profiles_unique', r.discovery_profiles_unique,
    'enrichment_status', r.enrichment_status,
    'enrichment_profiles_total', r.enrichment_profiles_total,
    'enrichment_profiles_completed', r.enrichment_profiles_completed,
    'enrichment_profiles_failed', r.enrichment_profiles_failed,
    -- Campos compatíveis com Google Maps
    'found_quantity', r.discovery_profiles_found,
    'created_quantity', r.created_quantity,
    'duplicates_skipped', r.duplicates_skipped,
    'filtered_out', 0, -- Instagram não tem filtros
    'pages_consumed', r.discovery_queries_completed,
    'credits_consumed', r.credits_consumed,
    'started_at', r.started_at,
    'finished_at', r.finished_at,
    'discovery_completed_at', r.discovery_completed_at,
    'enrichment_completed_at', r.enrichment_completed_at,
    'execution_time_ms', r.execution_time_ms,
    'duration_formatted', CASE
      WHEN r.execution_time_ms IS NOT NULL THEN
        CASE
  ...
  LEFT JOIN funnel_columns fc ON fc.id = r.column_id
  WHERE r.id = v_run_id;

  -- ========================================
  -- TIMELINE DE LOGS (usa extraction_logs com source='instagram')
  -- ========================================
  SELECT COALESCE(json_agg(
    json_build_object(
      'timestamp', TO_CHAR(el.created_at, 'HH24:MI:SS'),
  ...
  INTO v_timeline
  FROM extraction_logs el
  WHERE el.run_id = v_run_id AND el.source = 'instagram';

  -- ========================================
  -- CONTADORES DE PERFIS
  -- ========================================
  SELECT
    COUNT(*),
    COUNT(*) FILTER (WHERE processing_status = 'completed' OR migrated_to_leads = true),
    COUNT(*) FILTER (WHERE processing_status = 'pending'),
    COUNT(*) FILTER (WHERE processing_status = 'failed'),
    COUNT(*) FILTER (WHERE processing_status = 'skipped'),
    COUNT(*) FILTER (WHERE is_verified = true),
    COUNT(*) FILTER (WHERE is_business_account = true),
    COUNT(*) FILTER (WHERE is_professional_account = true),
    COUNT(*) FILTER (WHERE is_private = true),
    COUNT(*) FILTER (WHERE is_private = false OR is_private IS NULL)
  INTO
    v_total_profiles, v_profiles_migrated, v_profiles_pending,
    v_profiles_failed, v_profiles_skipped,
    v_verified_count, v_business_count, v_professional_count,
    v_private_count, v_public_count
  FROM instagram_enriched_profiles
  WHERE run_id = v_run_id;

  -- ========================================
  -- CONTADORES DE CONTATO (apenas perfis migrados)
  -- ========================================
  SELECT
    COUNT(*) FILTER (WHERE
      business_email IS NOT NULL AND business_email != ''
      OR email_from_bio IS NOT NULL AND email_from_bio != ''
    ),
    COUNT(*) FILTER (WHERE
      business_phone IS NOT NULL AND business_phone != ''
      OR phone_from_bio IS NOT NULL AND phone_from_bio != ''
    ),
    COUNT(*) FILTER (WHERE
      external_url IS NOT NULL AND external_url != ''
    ),
    COUNT(*) FILTER (WHERE
      whatsapp_from_bio IS NOT NULL AND whatsapp_from_bio != ''
    ),
    COUNT(*) FILTER (WHERE
      email_from_bio IS NOT NULL AND email_from_bio != ''
    ),
    COUNT(*) FILTER (WHERE
      phone_from_bio IS NOT NULL AND phone_from_bio != ''
    )
  INTO
    v_with_email, v_with_phone, v_with_website, v_with_whatsapp,
    v_email_from_bio, v_phone_from_bio
  FROM instagram_enriched_profiles
  WHERE run_id = v_run_id
    AND (processing_status = 'completed' OR migrated_to_leads = true);

  -- ========================================
  -- MÉTRICAS DE SEGUIDORES
  -- ========================================
  SELECT
    COALESCE(AVG(followers_count), 0),
    COALESCE(MAX(followers_count), 0),
    COALESCE(MIN(followers_count), 0)
  INTO v_avg_followers, v_max_followers, v_min_followers
  FROM instagram_enriched_profiles
  WHERE run_id = v_run_id
    AND followers_count IS NOT NULL;

  -- ========================================
  -- MONTAR OBJETO CONTATOS (compatível com Google Maps)
  -- ========================================
  v_contatos := json_build_object(
    'total', v_profiles_migrated,
    'telefone', json_build_object(
      'com', v_with_phone,
      'percentual', CASE WHEN v_profiles_migrated > 0 THEN ROUND((v_with_phone::NUMERIC / v_profiles_migrated) * 100, 1) ELSE 0 END
    ),
    'tipo_telefone', json_build_object(
      'fixo', 0, -- Instagram geralmente não tem telefone fixo
      'celular', v_with_phone
    ),
    'website', json_build_object(
      'com', v_with_website,
      'percentual', CASE WHEN v_profiles_migrated > 0 THEN ROUND((v_with_website::NUMERIC / v_profiles_migrated) * 100, 1) ELSE 0 END,
      'brasileiro', 0,
      'internacional', v_with_website
    ),
    'email', json_build_object(
      'com', v_with_email,
      'percentual', CASE WHEN v_profiles_migrated > 0 THEN ROUND((v_with_email::NUMERIC / v_profiles_migrated) * 100, 1) ELSE 0 END,
      'da_bio', v_email_from_bio,
      'business', v_with_email - v_email_from_bio
    ),
    'cnpj', json_build_object(
      'com', 0, -- Instagram não tem CNPJ
      'percentual', 0
    ),
    'whatsapp', json_build_object(
      'valido', v_with_whatsapp,
      'verificado', v_with_whatsapp,
      'da_bio', v_with_whatsapp
    ),
    'localizacao', json_build_object(
      'com_endereco', 0
    )
  ...
  -- ========================================
  -- MONTAR OBJETO PERFIS (específico Instagram)
  -- ========================================
  v_perfis := json_build_object(
    'total', v_total_profiles,
    'migrados', v_profiles_migrated,
    'pendentes', v_profiles_pending,
    'falhas', v_profiles_failed,
    'pulados', v_profiles_skipped,
    'tipos', json_build_object(
      'verificados', v_verified_count,
      'empresas', v_business_count,
      'profissionais', v_professional_count,
      'privados', v_private_count,
      'publicos', v_public_count
    ),
    'seguidores', json_build_object(
      'media', ROUND(v_avg_followers, 0),
      'maximo', v_max_followers,
      'minimo', v_min_followers
    )
  );
  ...
  v_graficos := json_build_object(
    'pizza_contatos', json_build_array(
      json_build_object('name', 'Com Email', 'value', v_with_email),
      json_build_object('name', 'Com Telefone', 'value', v_with_phone),
      json_build_object('name', 'Com Website', 'value', v_with_website),
      json_build_object('name', 'Com WhatsApp', 'value', v_with_whatsapp)
    ),
    'pizza_tipos_perfil', json_build_array(
      json_build_object('name', 'Empresas', 'value', v_business_count),
      json_build_object('name', 'Profissionais', 'value', v_professional_count),
      json_build_object('name', 'Verificados', 'value', v_verified_count),
      json_build_object('name', 'Públicos', 'value', v_public_count - v_business_count - v_professional_count)
    ),
    'pizza_status', json_build_array(
      json_build_object('name', 'Migrados', 'value', v_profiles_migrated),
      json_build_object('name', 'Pendentes', 'value', v_profiles_pending),
      json_build_object('name', 'Falhas', 'value', v_profiles_failed),
      json_build_object('name', 'Pulados', 'value', v_profiles_skipped)
    ),
    'pizza_whatsapp', json_build_array(
      json_build_object('name', 'Com WhatsApp', 'value', v_with_whatsapp),
      json_build_object('name', 'Sem WhatsApp', 'value', GREATEST(v_profiles_migrated - v_with_whatsapp, 0))
    ),
    'pizza_website', json_build_array(
      json_build_object('name', 'Com Website', 'value', v_with_website),
      json_build_object('name', 'Sem Website', 'value', GREATEST(v_profiles_migrated - v_with_website, 0))
    )
  );
  ...
  v_result := json_build_object(
    ...
    'contatos', v_contatos,
    'perfis', v_perfis,
    'enriquecimento', json_build_object(
      'whois', json_build_object('enriched', 0, 'pending', 0),
      'cnpj', json_build_object('enriched', 0, 'pending', 0),
      'scraping', json_build_object('completed', v_profiles_migrated, 'pending', v_profiles_pending),
      'whatsapp', json_build_object('checked', v_with_whatsapp, 'pending', 0),
      'total', v_total_profiles
    ),
    'qualidade', json_build_object(
      'total_leads', v_total_profiles,
      'qualificados', v_profiles_migrated,
      'rejeitados', v_profiles_failed + v_profiles_skipped,
      'leads_criados', v_profiles_migrated,
      'taxa_qualificacao', CASE WHEN v_total_profiles > 0
        THEN ROUND((v_profiles_migrated::NUMERIC / v_total_profiles) * 100, 1)
        ELSE 0
      END,
      'classificacao', CASE
        WHEN v_profiles_migrated > 0 AND (v_with_email + v_with_phone + v_with_website) / v_profiles_migrated::NUMERIC > 0.5 THEN 'Boa'
        WHEN v_profiles_migrated > 0 THEN 'Média'
        ELSE '-'
      END,
      'score_medio', CASE WHEN v_profiles_migrated > 0
        THEN ROUND(((v_with_email + v_with_phone + v_with_website + v_with_whatsapp)::NUMERIC / (v_profiles_migrated * 4)) * 100, 1)
        ELSE 0
      END,
      'estrelas', CASE
        WHEN v_profiles_migrated = 0 THEN 0
        WHEN (v_with_email + v_with_phone + v_with_website) / v_profiles_migrated::NUMERIC > 0.7 THEN 5
        WHEN (v_with_email + v_with_phone + v_with_website) / v_profiles_migrated::NUMERIC > 0.5 THEN 4
        WHEN (v_with_email + v_with_phone + v_with_website) / v_profiles_migrated::NUMERIC > 0.3 THEN 3
        WHEN (v_with_email + v_with_phone + v_with_website) / v_profiles_migrated::NUMERIC > 0.1 THEN 2
        ELSE 1
      END,
      'distribuicao', json_build_object(
        'excelente', 0,
        'alta', 0,
        'media', v_profiles_migrated,
        'baixa', 0
      )
    ),
    'graficos', v_graficos,
...
-- Conceder permissões
GRANT EXECUTE ON FUNCTION get_instagram_extraction_analytics(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION get_instagram_extraction_analytics(UUID) TO service_role;

-- Comentários
COMMENT ON FUNCTION get_extraction_analytics IS 'Analytics para extrações Google Maps/CNPJ. Retorna métricas de leads, contatos, enriquecimento, etc.';
COMMENT ON FUNCTION get_instagram_extraction_analytics IS 'Analytics específico para extrações do Instagram. Retorna métricas de perfis, seguidores, contatos extraídos, etc.';
