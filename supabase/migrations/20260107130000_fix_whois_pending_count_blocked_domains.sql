-- =============================================================================
-- CORREÇÃO: Excluir domínios bloqueados da contagem de WHOIS pendentes
-- =============================================================================
-- Problema: Domínios como ifood.com.br, instagram.com.br eram contados como
-- "pendentes" mesmo estando na lista de bloqueados do process-whois-queue
--
-- Solução: Criar array de domínios bloqueados e excluir da contagem
-- =============================================================================
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (WHERE
      domain IS NOT NULL
      AND domain LIKE '%.br'
      AND domain != ALL(v_blocked_domains)  -- ✅ Excluir bloqueados EXATOS
      AND NOT EXISTS (  -- ✅ Excluir subdomínios bloqueados (ex: shop.ifood.com.br)
    ...
    -- Pendentes (tem .br, NÃO bloqueado, não enriquecido)
    COUNT(*) FILTER (WHERE
      domain IS NOT NULL
      AND domain LIKE '%.br'
      AND domain != ALL(v_blocked_domains)  -- ✅ CORREÇÃO PRINCIPAL
      AND NOT EXISTS (
        SELECT 1 FROM unnest(v_blocked_domains) AS blocked
        WHERE domain LIKE '%.' || blocked
      )
...
'Retorna status de enriquecimento excluindo domínios bloqueados (marketplaces, redes sociais) da contagem de WHOIS pendentes';
