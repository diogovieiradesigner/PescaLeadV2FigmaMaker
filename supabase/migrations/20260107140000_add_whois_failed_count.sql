-- =============================================================================
-- MELHORIA: Adicionar contagem de "failed" ao WHOIS (igual ao Scraping)
-- =============================================================================
-- Motivo: Melhorar UX mostrando "Falhas" ao invés de "Pendentes" para domínios
-- que foram consultados mas retornaram 404/erro
--
-- Comportamento:
-- - pending: Ainda não foi consultado (whois_checked_at IS NULL)
-- - failed: Foi consultado mas retornou erro (whois_checked_at NOT NULL AND whois_enriched = false)
-- - enriched: Sucesso
-- =============================================================================
...
DECLARE
  ...
  -- Lista de domínios bloqueados (sync com process-whois-queue + website-validator)
  v_blocked_domains TEXT[] := ARRAY[
    -- Redes Sociais
    'instagram.com.br', 'facebook.com.br', 'linkedin.com.br',
    'youtube.com.br', 'tiktok.com.br',
    ...
    'shopee.com.br', 'aliexpress.com.br',
    'ifood.com.br', 'rappi.com.br', 'ubereats.com.br',
    'sympla.com.br',

    -- Agregadores de Links
    'linktr.ee', 'linktree.com.br', 'bio.link',

    -- Google Services
    'business.google.com.br', 'g.page',

    -- Site Builders
    'wixsite.com.br', 'wordpress.com.br', 'blogspot.com.br',

    -- Comunicação
    'whatsapp.com.br', 'telegram.me'
  ];
...
BEGIN
  ...
  IF v_total = 0 THEN
    RETURN json_build_object(
      ...
      'whois', json_build_object('total_br', 0, 'enriched', 0, 'pending', 0, 'failed', 0, 'internacional', 0, 'sem_dominio', 0),
  ...
  -- ========================================
  -- WHOIS Stats (COM "failed")
  -- ========================================
  SELECT
    -- Total .br (tem domínio .br E NÃO está bloqueado)
    COUNT(*) FILTER (WHERE
      domain IS NOT NULL
      AND domain LIKE '%.br'
      AND domain != ALL(v_blocked_domains)
      AND NOT EXISTS (
        SELECT 1 FROM unnest(v_blocked_domains) AS blocked
        WHERE domain LIKE '%.' || blocked
      )
    ),
    -- Enriquecidos
    COUNT(*) FILTER (WHERE
    ...
    -- Pendentes (NÃO bloqueado, NÃO consultado ainda)
    COUNT(*) FILTER (WHERE
      domain IS NOT NULL
      AND domain LIKE '%.br'
      AND domain != ALL(v_blocked_domains)
      ...
      AND whois_checked_at IS NULL  -- ✅ Ainda não consultou
    ),
    -- ✅ NOVO: Falhas (foi consultado mas retornou erro)
    COUNT(*) FILTER (WHERE
      domain IS NOT NULL
      AND domain LIKE '%.br'
      AND domain != ALL(v_blocked_domains)
      ...
      AND whois_checked_at IS NOT NULL  -- ✅ Já consultou
      AND whois_enriched = false         -- ✅ Mas falhou
    ),
  ...
  RETURN json_build_object(
    ...
    'whois', json_build_object(
      ...
      'failed', v_whois_failed,  -- ✅ NOVO CAMPO
...
COMMENT ON FUNCTION get_enrichment_status(UUID) IS
'Retorna status de enriquecimento com contagem de falhas WHOIS (domínios consultados mas com erro 404)';
