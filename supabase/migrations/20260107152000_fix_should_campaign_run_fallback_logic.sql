-- =============================================================================
-- FIX: should_campaign_run() - Corrigir lógica de fallback
-- =============================================================================
-- Bug encontrado: Após fazer switch para reserva, estava retornando variáveis
-- antigas (instance_status e instance_name do principal desconectado)
--
-- Correção: Após switch, atualizar variáveis com dados da reserva selecionada
-- =============================================================================
...
BEGIN
    ...
    -- ========================================
    -- Buscar inbox principal via campaign_instance_config
    -- ========================================
    ...
    IF v_instance_status != 'connected' THEN
        ...
        IF v_fallback_enabled THEN
            ...
            IF v_fallback_inbox.inbox_id IS NOT NULL THEN
                -- ✅ FIX: Atualizar TODAS as variáveis com dados da reserva
    ...
    WHERE (
      ...
      AND cr.config_id != p_config_id  -- ✅ FIX: Não bloquear a própria config
    LIMIT 1;
    ...
    RETURN jsonb_build_object(
        ...
        'instance_status', v_instance_status,  -- ✅ Agora reflete a reserva se houver switch
        'instance_name', v_instance_name,      -- ✅ Agora reflete a reserva se houver switch
        'inbox_id', v_inbox_id,
        'is_fallback', v_inbox_id != v_primary_inbox_id,
...
'Verifica se campanha deve rodar com suporte a fallback multi-WhatsApp. Corrigido switch automático em 2026-01-07.';
