-- =============================================================================
-- Migration: Fix extraction analytics contact data counting
-- =============================================================================
-- PROBLEMA: Os cards de "DADOS DE CONTATO" mostram 0 porque a RPC usa
-- `migrated_at IS NOT NULL` mas para Google Maps esse campo não é preenchido.
--
-- SOLUÇÃO: Usar `filter_passed = true` OU `migrated_at IS NOT NULL` para
-- considerar leads criados, já que para Google Maps os leads são inseridos
-- diretamente sem processo de migração separado.
-- =============================================================================
...
DECLARE
  ...
  -- Contadores de contato (APENAS leads criados - usando filter_passed OU migrated_at)
  v_com_telefone INT;
  v_telefone_fixo INT;
  v_com_website INT;
  v_sites_br INT;
...
BEGIN
  ...
  -- ========================================
  -- CONTADORES DE CONTATO (LEADS CRIADOS)
  -- FIX: Usar filter_passed = true OU migrated_at IS NOT NULL
  -- Isso cobre tanto Google Maps (filter_passed) quanto CNPJ/Instagram (migrated_at)
  -- ========================================
  WITH leads_criados AS (
    SELECT *
    FROM lead_extraction_staging
    WHERE extraction_run_id = v_run_id
      AND (filter_passed = true OR migrated_at IS NOT NULL)
  )
  SELECT
    ...
    -- Com endereço
    COUNT(*) FILTER (WHERE
      extracted_data->>'address' IS NOT NULL AND extracted_data->>'address' != ''
      OR extracted_data->>'city' IS NOT NULL
      OR extracted_data->>'state' IS NOT NULL
    )
  INTO
  ...
  -- Contar leads criados (usando mesma lógica)
  ...
  -- Determinar classificação e estrelas
...
-- Comentário
COMMENT ON FUNCTION get_extraction_analytics IS 'Analytics para extrações Google Maps/CNPJ. FIX: Usa filter_passed=true OU migrated_at IS NOT NULL para contar leads criados.';
